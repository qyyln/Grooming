...
public class main {
	...
	public boolean addLightPath(int nodei, int nodej, int capacity, boolean takeTransponder, ArrayList<Integer> path) {
		...
		while(channelNeeded>0){
			...
			boolean isSpaceFree = spectrumSearch(nodei,nodej,currentChannelNeeded,path); //you liang ge fanhuizhi
			ArrayList<Integer> channelSet = new ArrayList<>();
			if(!isSpaceFree)return false;
			spectrumTake(nodei,nodej,channelSet,path);
			boolean isNewlyReserve = spectrumReserve(nodei,nodej,currentChannelNeeded,path,"l");//you liang ge fanhuizhi
			ArrayList<Object> list = new ArrayList<>();
			list.add(path);
			...
			list.add(channelSet);
			list.add("l");
			lightPathRes.put(nodei*100+nodej,list);
			if(isNewlyReserve){
				while(isNewlyReserve&& NewlyReserve){
					System.out.println("newlyreserve",NewlyReserve);
					if(currentChannelNeeded>substChannel){
						...
						int subrx = 0;
						if(takeTransponder){
							subtxFound,transpondertx,subtx=subTransponderSearch(nodei);
							if(!subtxFound)return false;
							subrxFound,transponderrx,subrx=subTransponderSearch(nodej);
						...
						ArrayList<Object> temp = spectrumRes.get(nodei*100+nodej);
						for(int i=NewlyReserve;i<temp.size();i++){
							temp.set(2,substChannel);
						}
						NewlyReserve = NewlyReserve-1;
						currentChannelNeeded -= substChannel;
						lightPathRes.get(nodei*100+nodej).add("subtx");
						ArrayList<Object> tx = new ArrayList<>();
						tx.add(nodei);
						tx.add(transpondertx);
						tx.add(subtx);
						lightPathRes.get(nodei*100+nodej).add(tx);
						lightPathRes.get(nodei*100+nodej).add("subrx");
						ArrayList<Object> rx = new ArrayList<>();
						rx.add(nodei);
						rx.add(transponderrx);
						rx.add(subrx);
						lightPathRes.get(nodei*100+nodej).add(rx);
					}
					else{
						boolean subtxFound = false;
						int transpondertx = 0;
						...
						ArrayList<Object> temp = spectrumRes.get(nodei*100+nodej);
						for(int i=NewlyReserve;i<temp.size();i++){
							temp.set(2,currentChannelNeeded);
						}
						NewlyReserve = NewlyReserve-1;
